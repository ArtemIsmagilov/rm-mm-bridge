...
def create_app(test_config=None):
    ...
    def generate_priorities_for_form(redmine):
        result = [{"label": p.name, "value": f'{p.id}'} for p in
    ...
    def app_info_handler():
        logging.info('%s', request.json)
        context = request.json['context']
        login_in_mattermost = context['acting_user']['username']
        md = render_template('app_info.md', login_in_mattermost=login_in_mattermost)
        ...
        with Redmine(redmine_url, key=ADMIN_RM_KEY_API).session() as redmine:
            for u in parsed_data:
                ...
                with redmine.session(impersonate=login_in_redmine_next_user):
                    ...
                    # validation2 [have account and token for REST API]
                    redmine_user = check_exist_account_and_token_in_redmine(redmine, u.login,
                                                                            login_in_redmine_next_user)
    ...
    def create_direct_channel(context):
